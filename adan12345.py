...
def move_balls(SCREEN_WIDTH,SCREEN_HEIGHT,BALLS):
    for ball in BALLS:
        ...
        ball.goto(random.randint(round(-SCREEN_WIDTH + RADIUS) , round(SCREEN_WIDTH-RADIUS)),random.randint(round(-SCREEN_HEIGHT + RADIUS) , round(SCREEN_HEIGHT - RADIUS)))
...
while RUNNING == True:
    move_balls(SCREEN_WIDTH,SCREEN_HEIGHT,BALLS)
    check_all_balls_collision()
    MY_BALL.move(round(SCREEN_WIDTH), round(SCREEN_HEIGHT))
    
    if RUNNING == check_myball_collision():
